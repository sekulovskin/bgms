...
- **Exception**: inside function-call arguments where the result must be
  captured, use `<-`. With `=` R treats it as a named argument. Example:
  `expect_message(result <- foo(), "pattern")`.
...
- `@examples` must be runnable by `R CMD check`. Use `\donttest{}` for
  slow examples. Use `\dontrun{}` only when the example truly cannot run
  in a clean R session.
- Add `@family` tags: `model-fitting`, `posterior-methods`,
  `prediction`, `extractors`, `diagnostics`.
...
Before every commit that touches R code, run these checks and fix any
issues they report:
...
2.  **Lint** — catch issues the CI lint workflow will flag:
    ...
    `seq_linter()` (among defaults). Fix all findings before committing.
...
4.  **R CMD check** — if the change is non-trivial, run:
    ...
    CI uses `error-on = "warning"`, so warnings are treated as failures.
...
- Write session-oriented documentation (“in your R session”, “you should
  see”, “run this in your console”). Documentation describes what a
  function does and returns, not what the reader should do
  interactively.
